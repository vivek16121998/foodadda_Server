...
public class AdminAPI {
	...
	public ResponseEntity<?> showAllRestaurants() throws Exception{
			...
			logger.info("Fetch all the restaurants currently operating");
			List<RestaurantDTO> restaurantList = adminService.getRestaurant();
	...
	public ResponseEntity<?> ratingsBasedRestaurant(@PathVariable("rating") Float rating) throws Exception{
			...
			logger.info("Fetch all the restaurants according to the ratings provided");
			List<RestaurantDTO> restaurantList = adminService.filterRestaurantOnRatings(rating);
			return new ResponseEntity<List<RestaurantDTO>>(restaurantList, HttpStatus.OK);
	...
	public ResponseEntity<String> grantOrRejectPermission(@PathVariable String status,
			@RequestBody RestaurantDTO restaurant) throws Exception{
